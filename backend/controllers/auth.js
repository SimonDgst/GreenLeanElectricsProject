...
const Auth = require('../models/prosumer');
...
exports.signup = (req, res, next) => {
    bcrypt.hash(req.body.password, 10) //the higher salt is the higher the password will be secured
        .then(hash => {
            const prosumer = new Auth({
                email: req.body.email,
                password: hash
            });
            prosumer.save()
                .then(() => res.status(201).json({ message: 'Auth account created !' }))
...
exports.login = (req, res, next) => {
    Auth.findOne({ email: req.body.email }) //Look into the database if the email is saved
        .then(prosumer => {
            if (!prosumer) {
                return res.status(401).json({ error: 'Auth account not found' });
            }
